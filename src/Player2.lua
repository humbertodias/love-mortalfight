...
function Player2:init()
    ...
    -- player 2 settings like name, position, velocity
...
function Player2:move(direction)
...
function Player2:update(dt)
    ...
    if not self.died and game.started then
        ...
        elseif love.keyboard.isDown('d') and math.abs(distance()) < 400 then
           
            -- move character when not close the edge
           
            if self.x < arena.rightSide then
                self.x = self.x + self.walkSpeed * dt
            end
        ...
        if self.kick then
            ...
            if math.floor(distance()) < self.kickDistance then
                ...
                if not player1.died and player1.block then
                    player1.health = player1.health - (6 * dt)
                    self.kick = false

                -- if player not blocking more damage 

                elseif not player1.died and not player1.block then 
                    player1.health = player1.health - (math.random(6,self.kickStrenght) * dt)
                    self.kick = false
                end
        ...
        if self.punch then
            ...
            if math.floor(distance()) < self.punchDistance then
                ...
                if not player1.died and player1.block then
                    player1.health = player1.health - (5 * dt)
                    self.punch = false

                -- if other player is not blocking then more damage 

                elseif not player1.died and not player1.block then 
                    player1.health = player1.health - (math.random(5,self.punchStrenght) * dt)
                    self.punch = false
                end
...
function Player2:render()
    ...
    love.graphics.draw(gImage['janusz-full'], gFrame['janusz-full'][self.currentAnimation:getCurrentFrame()], self.x, self.y,0, self.direction == 'left' and 1 or -1, 1, self.width / 2)
end
